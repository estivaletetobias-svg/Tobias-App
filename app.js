    ...
    function initEventListeners() {
        ...
        document.getElementById('start-session-btn')?.addEventListener('click', async () => {
            ...
            if (result && result.success && result.data && result.data.exercises?.length > 0) {
                ...
                // Em caso de falha completa da IA ou banco vazio, injetar um treino de fallback
                // para não quebrar a UI
                window.todayExercises = [
                    { name: "Aquecimento Geral", sets: 1, reps: "5-10 min", cues: "Prepare o corpo", weight: "0kg", rest_sec: 0 }
                ];
            }
